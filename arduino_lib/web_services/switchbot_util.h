...
void hmac_sha256(const char *p_key, const char *p_payload,
                 unsigned char *p_hmacResult) {
  mbedtls_md_context_t ctx;
  mbedtls_md_type_t md_type = MBEDTLS_MD_SHA256;
  mbedtls_md_init(&ctx);
  mbedtls_md_setup(&ctx, mbedtls_md_info_from_type(md_type), 1);
  mbedtls_md_hmac_starts(&ctx, (const unsigned char *)p_key, strlen(p_key));
  mbedtls_md_hmac_update(&ctx, (const unsigned char *)p_payload,
                         strlen(p_payload));
  mbedtls_md_hmac_finish(&ctx, p_hmacResult);  // 32 bytes
  mbedtls_md_free(&ctx);
}

std::tuple<String, String, String> make_switchbot_sign(String &token,
                                                       String &secret) {
  String nonce = "";
...
std::optional<String> get_device_status(String &token, String &secret,
                                        String &device_id) {
  HTTPClient http;
  String url =
...
std::optional<String> send_device_command(String &token, String &secret,
                                          String &device_id,
                                          String &command_type,
                                          String &command) {
  HTTPClient http;
