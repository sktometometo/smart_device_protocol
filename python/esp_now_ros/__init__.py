...
class MetaFrame:
    ...
    def to_bytes(self) -> bytes:
        ...
        for i in range(3):
            if i < len(self.interface_descriptions):
                ...
                data += struct.pack("64s", packet_description.encode("utf-8")) + struct.pack("10s", serialization_format.encode("utf-8"))
            else:
                data += b"\x00" * 74
        return data
    ...
    def from_bytes(data: bytes):
        ...
        device_name = struct.unpack("20s", data[1:1+20])[0].decode("utf-8").replace("\x00", "")
        interface_descriptions = []
        for i in range(3):
            packet_description = struct.unpack("64s", data[21+i*74:21+64+i*74])[0].decode("utf-8").replace("\x00", "")
            serialization_format = struct.unpack("10s", data[21+64+i*74:21+64+10+i*74])[0].decode("utf-8").replace("\x00", "")
            if len(packet_description) > 0 and len(serialization_format) > 0:
                interface_descriptions.append((packet_description, serialization_format))
        return MetaFrame(device_name=device_name, interface_descriptions=interface_descriptions)
        


class DataFrame:

    def __init__(self, packet_description: str, content: List[Union[bool, int, float, str]]):
        ...
        for entry in content:
            if type(entry) is bool:
                ...
                if len(encoded_entry) <= 16:
                    ...
                    raise ValueError(f"String entry of content is longer than 64 bytes: {len(encoded_entry)}")
            else:
    ...
    def to_bytes(self) -> bytes:
        data: bytes = struct.pack("<B", PACKET_TYPE_DATA) + struct.pack("64s", self.packet_description.encode("utf-8")) + struct.pack("10s", self.serialization_format.encode("utf-8"))
    ...
    def from_bytes(data: bytes):
        ...
        packet_description = struct.unpack("64s", data[1:1+64])[0].decode("utf-8").replace("\x00", "")
        serialization_format = struct.unpack("10s", data[1+64:1+64+10])[0].decode("utf-8").replace("\x00", "")
        ...
        for format_specifier in serialization_format:
            if format_specifier == "?":
                ...
                entry = struct.unpack("16s", data[index:index+16])[0].decode("utf-8").replace("\x00", "")
                content.append(entry)
                index += 16
            elif format_specifier == "s":
                entry = struct.unpack("64s", data[index:index+64])[0].decode("utf-8").replace("\x00", "")
                content.append(entry)
                index += 64
            else:
