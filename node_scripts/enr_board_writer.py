...
def main(target_address:str, message: str, source_name: str = 'sample_source', timeout_duration: int = 60000):
  
  rospy.init_node('esp_board_writer')
  interface = ESPNOWROSInterface()
  rospy.sleep(5.)
  addr = list(map(lambda x: int(x, 16), target_address.split(':')))
  packet = create_device_message_board_data(source_name=source_name, timeout_duration=timeout_duration, message=message)
